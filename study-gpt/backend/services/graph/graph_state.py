...
from typing import TypedDict, Any #딕셔너리 구조 타입 정의
...
class GraphState(TypedDict): #GraphState 는 반드시 이런 값들을 가진다고 정의
    ...
    level: str
    response: str
    curriculum: list #현재 생성된 학습 커리큘럼 저장 공간
    current_step: dict #현재 사용자가 학습 중인 step 정보 저장 공간
    learning_intent: str #현재 학습 흐름 의도를 저장하는 상태값
    user_id: int #현재 로그인 사용자
    room_id: int #현재 채팅방
    db: object #GraphState 안에 SQLAlchemy DB 연결 객체 저장
    learning_status: str #현재 사용자의 학습 상태를 기억하기 위한 값 (학습중, 일시정지, 학습종료)
    vector_store: Any
