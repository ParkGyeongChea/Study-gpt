...
기존 학습 흐름이 끊어져 버린다는 점이었습니다.

이를 해결하기 위해 Study GPT는 별도의 Side Chat 기능을 제공합니다.
...
OpenAI API 호출 비용 관리와 악의적 사용 방지를 고려하여 현재는 로그인 사용자에게만 AI 기능을 제공하고 있습니다.
...
시스템이 학습 의도를 분석한 뒤 커리큘럼을 생성합니다.

이후 현재 학습 단계(current_step)를 기준으로
설명, 복습, 추가 질문을 처리하며,
사용자는 중단된 학습을 이어서 진행할 수 있습니다.

또한 PDF, TXT 문서를 업로드하면
문서 내용을 기반으로 요약, 질의응답, 설명 생성이 가능합니다.
...
같은 질문이 가능하며,
현재 학습 단계(current_step)를 기준으로, 
설명 흐름을 유지한 상태에서 답변을 제공합니다.
...
업로드된 문서는 텍스트를 추출한 뒤 벡터 데이터베이스(FAISS)에 저장되며, 이를 기반으로 문서 요약, 문서 질의응답, 문서 기반 학습 기능을 제공합니다.
